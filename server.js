...
const server = http.createServer((req, res) => {
    ...
    if (pathname === "/data" && req.method === "GET") {
        const dataPath = path.join(__dirname, "uploads", "data.xlsx");
        if (!fs.existsSync(dataPath)) { res.writeHead(404); res.end("Ingen Excel fil fundet"); return; }
        try {
            const sheet = xlsx.readFile(dataPath).Sheets[xlsx.readFile(dataPath).SheetNames[0]];
            res.writeHead(200, { "Content-Type": "application/json" });
            res.end(JSON.stringify(xlsx.utils.sheet_to_json(sheet)));
        } catch (err) {
            res.writeHead(500); res.end("Kunne ikke læse Excel");
        }
        return;
    }
